		...
		try {
			...
			try {
				...
				if(klass.getSuperclass() != null && klass.getSuperclass() != klass) {
					...
					Bukkit.getLogger().log(Level.WARNING, "No method found with name: " + nsme.getMessage());
				}
			}
			if(setMethod != null) setMethod.invoke(instance, value);
		} catch(Exception e) {
			Bukkit.getLogger().log(Level.SEVERE, "Failed to assign value to instance member variable");
		}
	}
